...
//
// Props:
// title - holds the post's title // text - holds the text of the post
...
export default {
  ...
  computed: {
    allModalFieldsValid() {
      return (
        this.titleValidator.state &&
        this.categoryValidator.state &&
        this.imageUrlValidator.state &&
        this.textValidator.state
      );
    },
    categoryOptions() {
      let options = [{ value: null, text: "Please Select a Category" }];
      this.$store.getters.getCategories.forEach((cat) =>
        options.push({ value: cat.name, text: cat.name })
      );
      return options;
    },
  },
...
<template>
  <b-form>
    <b-form-group
        ...
        label="Title"
        label-for="title"
        :state="titleValidator.state"
        :invalid-feedback="titleValidator.invalidFeedback"
    ...
    <b-form-group
        ...
        label="Category"
        label-for="category"
        :state="categoryValidator.state"
        :invalid-feedback="categoryValidator.invalidFeedback"
    ...
    <b-form-group
        ...
        label="Image URL"
        label-for="imageUrl"
        :state="imageUrlValidator.state"
        :invalid-feedback="imageUrlValidator.invalidFeedback"
    ...
    <b-form-group
        ...
        label="Text"
        label-for="text"
        :state="textValidator.state"
        :invalid-feedback="textValidator.invalidFeedback"
    ...
    <b-button type="button" variant="secondary" @click="$emit('cancelPost')"
    >Cancel</b-button
    >
    <b-button
        type="button"
        variant="primary"
        :disabled="!allModalFieldsValid"
        @mouseover="validateAllFields"
        @click="$emit('submitPost', post)"
    >Submit</b-button
    >
  </b-form>
</template>
