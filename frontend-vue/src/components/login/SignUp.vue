// captures sign-up information for a new user. If all form fields are valid, then this component will initiate
// the sign-up process with cognito
//
// Events:
//   'success', { formData } - emitted if the user was signed-up successfully with cognito
//   'cancel' - emitted if the user clicked the forms cancel button
//   'error',{ code, message } - emitted if an error occured signing up with Cognito
<template>
    <b-card border-variant="primary" no-body>
        ...
        <b-card-body>
            <b-form>
                ...
                <b-button block size="lg" type="submit" variant="danger" @click="cancel">Cancel</b-button>
...
export default {
  ...
  methods: {
    ...
    doSignUp () {
      ...
      this.$store.dispatch('doSignUp', this.form)
        .then(data => {
