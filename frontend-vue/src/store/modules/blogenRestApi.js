...
const blogenRestApiModule = {
  ...
  getters: {
    ...
    getPostIndicesByIds: (state) => (threadId, postId) => {
      ...
      if (indices.ti >= 0) {
        ...
        indices.ci = state.posts[indices.ti].children.findIndex(
  ...
  mutations: {
    ...
    UPDATE_POST(state, { ti, ci, newPost }) {
      ...
      logger.debug(`ti:${ti} ci:${ci} postData`, newPost);
      if (ci >= 0) {
    ...
    DELETE_POST(state, { ti, ci }) {
      // logger.debug(`DELETE_POST pi:${ti} ci:${ci}`)
      if (ci >= 0) {
  ...
  actions: {
    ...
    fetchAndStoreCategories: ({ commit }, { pageNum = 0, pageLimit = 20 }) => {
      return axios
        ...
        .then((res) => {
          logger.debug(`fetchAndStoreCategories response:`, res.data);
    ...
    fetchCategories: (context, { pageNum = 0, pageLimit = 20 }) => {
      return axios
        ...
        .then((res) => {
          logger.debug(`fetchCategories response:`, res.data);
          return res.data;
        })
        .catch((error) => {
          handleAxiosError(error);
    ...
    createCategory: ({ commit }, categoryObj) => {
      logger.debug("create category with object:", categoryObj);
      return axios
        .post("/api/v1/categories", categoryObj)
        .then((res) => {
          logger.debug("createCategory response:", res.data);
    ...
    updateCategory: ({ commit }, categoryObj) => {
      logger.debug("update category with object:", categoryObj);
      delete categoryObj.categoryUrl;
      return axios
        .put(`/api/v1/categories/${categoryObj.oldName}`, categoryObj)
        .then((res) => {
          logger.debug("updateCategory response:", res.data);
    ...
    fetchPosts: ({ commit }, { pageNum, pageLimit, categoryName }) => {
      axios
        ...
        .then((res) => {
          logger.debug("fetchPosts response:", res.data);
    ...
    createPost: (context, { id, post }) => {
      ...
      axios
        .post(url, post)
        .then((res) => {
          logger.debug(`createPost url:${url} response:`, res.data);
          if (!id) {
            ...
            let indices = context.getters.getPostIndicesByIds(
              res.data.threadId,
              res.data.postId
            );
    ...
    updatePost: (context, { id, post }) => {
      ...
      axios
        .put(url, post)
        .then((res) => {
          logger.debug(`updatePost url:${url} response:`, res.data);
    ...
    deletePost: (context, id) => {
      ...
      axios
        .delete(url)
        .then((res) => {
          logger.debug(`deletePost url:${url} response:`, res.data);
    ...
    fetchPostsByUser: (context, { id, pageNum, pageLimit, categoryName }) => {
      ...
      return axios
        ...
        .then((res) => {
          logger.debug(`get posts for user:${id} response:`, res);
    ...
    fetchAndStoreAvatarFileNames: ({ commit }) => {
      return axios
        .get("/api/v1/avatars")
        .then((res) => {
          logger.debug("fetch avatar file name response:", res);
    ...
    registerNewUser: (context, userId) => {
      ...
      return axios
        .get(url)
        .then((res) => {
          logger.debug("register new user", userId, res.status);
          return res;
        })
        .catch((error) => {
          throw error;
