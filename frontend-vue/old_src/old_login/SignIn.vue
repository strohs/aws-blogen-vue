// this component is for capturing the sign-in information of a Blogen user. it
encapsulates the logic for validating // the sign-in form inputs and then emits
the inputs to the parent when the form's submit button is clicked // // Props:
// you may pass the following optional props that will be used to pre-fill the
input form // email // password // Emits: // ('newPassword') - this event is
emitted when cognito requires a new password to be entered // ('resetPassword',
email) - indicates the user wants to reset their password // ('signUp') -
indicates the Sign-Up link was clicked // ('error', { code, message }) - this
event is emitted when some other error from Cognito
<template>
  <b-card border-variant="primary" no-body>
    <b-card-header>
      <div class="row">
        ...
        <b-link class="ms-auto mx-2" @click="$emit('signUp')">
    ...
    <b-card-body>
      <b-form>
        ...
        >
          ...
          <div>
            <small v-if="forgotPassword === true"
              >Enter your email address above and click the Reset Password
              button</small
            >
          </div>
        </b-form-group>
...
export default {
  ...
  computed: {
    ...
    passwordValidText() {
      return vt.requiredFieldValidText(this.v$.form.password);
    },
    passwordInvalidText() {
      return vt.requiredFieldInvalidText(
  ...
  methods: {
    ...
    async doSignIn() {
      ...
      // login to Cognito
      this.$store
        .dispatch("doSignIn", evt)
        .then((userData) => {
          // check if any challenges were set, blogen can handle 'NEW_PASSWORD_REQUIRED'...for now
          if (userData.challengeName) {
            console.log(
              `challenge issued: ${userData.challengeName} with required attrs:`,
              userData.challengeParam.requiredAttributes
            );
            console.log(
              "user attributes:",
              userData.challengeParam.userAttributes
            );
