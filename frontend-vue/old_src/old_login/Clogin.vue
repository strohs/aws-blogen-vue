// This is the main login handling component for the Blogen website // it
handles user SignUp and SignIn and orchestrates the transitions between the
forms of the login process //
<template>
  <!-- LOGIN -->
  <section id="clogin">
    <div class="">
      <div class="row justify-content-center my-2">
        <app-status-alert
          v-bind="statusObj"
          @dismissed="statusObj.show = false"
        ></app-status-alert>
      </div>
      <div class="row my-2 no-gutters">
        <div class="col-md-6 mx-auto">
          <transition name="flip" mode="out-in">
            ...
            <b-card
              v-else-if="activeForm.forgotPassword"
              key="forgotPassword"
              no-body
            >
              <b-card-header
              ...
              <b-card-body>
                <appForgotPasswordForm
                  :email="user.email"
                  @validated="doForgotPasswordSubmit"
                >
                </appForgotPasswordForm>
              </b-card-body>
            </b-card>
            ...
            <!-- New Password Form -->
            <b-card
              v-else-if="activeForm.newPassword"
              key="newPassword"
              no-body
            >
              <b-card-header
...
export default {
  ...
  created() {
    if (this.loginState === "LOGOUT") {
      ...
      // if a status message was set, show it
  ...
  methods: {
    ...
    doForgotPasswordSubmit(evt) {
      ...
      this.$store
        .dispatch("doForgotPasswordSubmit", {
          email: this.user.email,
          confCode: evt.confCode,
    ...
    doChallenge(userAttrs) {
      ...
      this.$store
        .dispatch("doCognitoChallenge", {
          cogUserData: this.cogUser,
          userAttrs: userAttrs,
        })
